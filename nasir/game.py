...
for item in items:
    ...
    if tracker < 5:
        ...
        response = input("Wanna buy {} for {}".format(item.get('name'), item.get('price')))
        if response == 'y':
            tracker +=1
...
if spendlimit <= playerlimit:
    ...
    for product in products:
        ...
        if len(purchased) < 5:
            the player needs a way to select a product
            selection = input("Are you interested in buying {} at {} | yes or no?".format(product.get('name'), product.get('price')))
